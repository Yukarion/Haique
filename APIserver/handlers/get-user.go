...
func (c *Container) GetUser(ctx echo.Context) error {
	...
	var tmp_res models.InlineResponce200

	user_id_str = ctx.Param("user_id")
	user_id, _ := strconv.Atoi(user_id_str)
	temp_res.User.UserId = int64(user_id)

	tmp_res.User.Name = c.RedisClient.Get(ctxBG, "user_id:"+user_id_str+":name").Result()

	subscription_id_str_list := c.RedisClient.Get(ctxBG, "user_id:"+user_id_str+":subscription").Result()
	...
	subscribed_by_id_str_list := c.RedisClient.Get(ctxBG, "user_id:"+user_id_str+":subscribed_by").Result()
	...
	author_haiku_id_str_list := c.RedisClient.Get(ctxBG, "user_id:"+user_id_str+":haiku_id_list").Result()
	...
	timeline_haiku_id_str_list := c.RedisClient.Get(ctxBG, "user_id:"+user_id_str+":haiku_id_list").Result()
	...
	for _, haiku_id_str := range haiku_id_str_list {
		...
		tmp_res.Haikus = append(temp_res.Haikus, tmp_haiku)
	}
